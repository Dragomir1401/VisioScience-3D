...
de unde se poate face lansarea sau oprirea lor în unități logice numite pod-uri. Kubernetes gestionează
...
Pentru fiecare componentă a backend‐ului (Deployment, Service, 
Ingress, ConfigMap, PersistentVolumeClaim) s-au creat fişiere YAML
separate în directoarele ./k8s/ ale fiecărui serviciu. Denumirea fișierelor corespunde resursei Kubernetes pe care o gestionează.
...
prometheus.yml) şi PVC este folosit de exemplu pentru volumul serviciului de vizualizare a metricilor în
...
metadatele şi specificațiile. Acestea pot fi aplicate cu kubectl apply -f <fișier>, sau
...
Clusterul este creat și gestionat după cum s-a prezentat anterior cu Kubernetes, folosind manifesturi YAML.
...
github.com/prometheus/client\_golang/prometheus pentru definirea contoarelor, 
histogramelor şi gauge-urilor.

github.com/prometheus/client\_golang/prometheus/promhttp pentru expunerea 
endpoint-ului /\$SERVICIU/metrics.
...
De exemplu după ce Prometheus trece prin evaluation\_service\_http\_requests\_total, se crează un dashboard 
...
Timeseries (serie temporală de date) pentru eval\_svc\_req\_total.

Gauge (serie de valori) pentru eval\_svc\_active\_evals.
...
(ex: \_id, class\_id) pentru a optimiza căutările și interogările.
