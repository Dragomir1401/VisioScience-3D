...
\section{Răspunsurile de posibili utilizatori}
\label{subsec:evaluare-backend-utilizatori}

În urma formularului discutat și la secțiunea motivație, au fost incluse și întrebări care dau o notă de la 1 la 5 pentru
fiecare tip de simulare 3D de la diferite materii din platformă în videoclipuri atașate. Au fost două tipuri de întrebări, 
una care cerea să se ofere o notă interactivității și una care cerea să se ofere o notă utilității simulărilor. S-a
înregistrat un număr de peste 35 de răspunsuri.

Pentru simulările de fizică peste 71\% dintre respondenți au dat nota maximă 5 pentru interactivitate și peste
68\% au dat nota maximă 5 pentru utilitate. Pentru exemplele de scene de chimie din videoclipurile atașate,
peste 64\% dintre respondenți au dat nota maximă 5 pentru interactivitate și peste 68\% au dat nota maximă 5 pentru utilitate.
Pentru simulările din zona de informatică, peste 71\% dintre respondenți au dat nota maximă 5 
pentru interactivitate și peste 65\% au dat nota maximă 5 pentru utilitate. De menționat este si că la toate
întrebările peste 95\% dintre respondenți au dat o notă mai mare sau egală cu 3/5 pentru interactivitate și utilitate la toate
materiile incluse.

În final, ca evaluare generală a platformei, am întrebat și dacă ar fi fost utilă o platformă ca aceasta dacă
respondenții ar fi avut acces la ea în școală. Rezultatele se pot vedea mai jos, cu peste 90\% dintre răspunsuri
indicând că sunt de acord sau foarte de acord că o platformă ca aceasta ar fi fost utilă.

\fig[width=0.8\textwidth]{imgs/ajutorForms.png}{fig:invatamatesjoc}{Ar fi fost utilă o platformă ca aceasta în școală?}
...
și manipularea datelor. Pentru aceasta, au fost utilizate soluții precum Postman\footnote{https://www.postman.com/}, curl\footnote{https://curl.se/}
și scripturi locale pentru a trimite cereri. În plus, au fost
testate diverse scenarii de utilizare pentru a verifica integritatea fluxului de date între componentele backend-ului și frontend. Acestea includ
inserarea, modificarea și ștergerea de date în MongoDB, precum și verificarea corectitudinii răspunsurilor API-urilor.

\subsection{Monitorizarea back-endului}
\label{subsubsec:evaluare-backend-monitorizare}

Pentru monitorizarea back-endului, s-au utilizat instrumente de adăugare și colectare precum Prometheus și Grafana pentru a
...
Performanța back-endului a fost evaluată pe baza unui set de testare a scenariilor de utilizare normale.
Testele au arătat o latență medie de răspuns de 250ms pe cereri simple și sub 2 secunde pentru încărcarea obiectelor mari din scene. În ceea ce
privește scalabilitatea, s-au făcut teste de încărcare pentru a evalua comportamentul sistemului la un număr mare de cereri simultane,
...
Alt nivel de testare în timpul dezvoltării a fost prin teste unitare,
rulate local cu go test. Folosind pachetul httptest am
simulat cereri HTTP către ruterul unui serviciu din interiorul podului şi am putut
...
regresiilor de exemplu din transformatorul de cod, parser sau din logica de
...
Testarea front-endului a fost realizată prin testare manuală a interacțiunilor cu utilizatorul. 
...
și integrarea corectă a modelelor în scene. Testele au arătat că 95\% dintre interacțiuni au fost realizate fără erori, iar platforma a funcționat
decent pe mașina de testare.
...
200MB pentru majoritatea scenelor 3D care încarcă obiecte și texturi.
...
S-au utilizat teste de scalabilitate, iar platforma a demonstrat un comportament robust și o scalabilitate bună
în fața unui număr mare de cereri. De exemplu, un scenariu de testare a implicat multiple cereri simultane, iar sistemul a rămas stabil,
...
specifice pentru Ingress. Ajustările făcute au asigurat că traficul extern poate accesa serviciile într-un mod stabil și mai ales controlat.
...
Cea mai dificilă parte a fost design-ul antetului tracer.h: au trebuit definite
meta-trăsături care să distingă între vector,  list,
map, priority_queue sau chiar std::array și altele, fără ajutorul
reflecţiei native asupra tipului. În plus, a fost nevoie de serializarea stării fiecărui 
container. Asta a complicat puternic manipularea tipurilor compuse, precum 
std::pair<K,V> din map-uri. Implementarea funcţiei auxiliare
serializePair() şi a ramurilor specializate din getState()
/ getMetadata() a solicitat multă muncă pentru a gestiona corect
...
în TransformCode() şi cele raportate de template-urile tracer a
implicat testare repetată, deoarece o singură pierdere genera stări incoerente la final.
...
de Kubernetes, a fost necesară crearea unui job\footnote{https://kubernetes.io/docs/concepts/workloads/controllers/job/}
dedicat care să aibă acces în interiorul
clusterului, deoarece nu se poate accesa direct din afară cu cereri
...
în mediul de dezvoltare, putând să difere în eventuala producție în funcție de încărcarea și performanțele serverului.
Rata de cadre pe secundă este asigurată de tehnologiile WebGL și Three.js folosite și de 
tehnicile de optimizare incluse. În general se menține constantă la cel puțin 60 cadre pe secundă.
...
se menține sub 200MB în majoritatea scenelor, ne încărcând resurse de foarte mari
dimensiuni.
...
şi depăşeşte nivelul minim de 60 cadre pe secundă, prag important pentru o
experienţă vizuală fluidă. În testele executate pe arhitectura backend-ului, aplicaţia a susţinut
până multiple cereri concurente fără să rescalaze pod-urile folosite.
Din punct de vedere monetar, lipsa taxei de licenţiere o poziţionează peste Mozaweb şi IQBoard prezentate la secțiunea Studiu
de piață, care introduc un cost de utilizare semnificativ.
