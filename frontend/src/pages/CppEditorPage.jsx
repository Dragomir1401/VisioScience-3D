...
const CppEditorPage = () => {
  ...
  const handleRun = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/cpp-compiler/compile-run",
        {
          method: "POST",
      ...
      if (data.error) {
        ...
        if (data.executionData) {
              ...
              switch (structureType) {
                ...
                case "unordered_map":
                  ...
                  if (structureType === "unordered_map") {
                    processedValue = calculateBuckets(
                      structureValue,
                      NUM_BUCKETS
                    );
                  }
          ...
          const firstStructure = Object.keys(newStructureStates).find(
            (varName) => {
              const structure = dataStructures.find(
                (ds) => ds.variableName === varName
              );
          ...
          if (firstStructure) {
            const foundStructure = dataStructures.find(
              (ds) => ds.variableName === firstStructure
            );
            if (foundStructure) {
              setSelectedStructure(foundStructure);
              setShowScene(true);
              console.log(
                "DEBUG: Automatically selected first structure:",
                foundStructure
              );
            }
          }
  ...
  const analyzeCode = (code) => {
    ...
    const patterns = {
      vector: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?vector\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
        description: "Dynamic array with contiguous storage",
      },
      list: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?list\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
        description: "Doubly-linked list",
      },
      forward_list: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?forward_list\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
        description: "Singly-linked list",
      },
      priority_queue: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?priority_queue\s*<(?:[^>]+|(?:[^,]+,\s*std::\w+<[^>]+>(?:,\s*std::(?:less|greater)<[^>]+>)?))>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "adaptor",
        description: "Priority queue",
      },
      deque: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?deque\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
        description: "Double-ended queue",
      },
      array: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?array\s*<[^,]+,\s*\d+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
      ...
      unordered_map: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?unordered_map\s*<[^,]+,\s*[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "unordered",
        description: "Hash table based map",
      },
      unordered_set: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?unordered_set\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "unordered",
        description: "Hash table based set",
      },
      unordered_multimap: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?unordered_multimap\s*<[^,]+,\s*[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "unordered",
        description: "Hash table based multimap",
      },
      unordered_multiset: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?unordered_multiset\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "unordered",
      ...
      map: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?(?<!unordered_)map\s*<[^,]+,\s*[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
        description: "Sorted key-value pairs",
      },
      set: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?(?<!unordered_)set\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
        description: "Sorted unique elements",
      },
      multimap: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?(?<!unordered_)multimap\s*<[^,]+,\s*[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
        description: "Sorted key-value pairs with duplicate keys",
      },
      multiset: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?(?<!unordered_)multiset\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "ordered",
      ...
      queue: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?queue\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "adaptor",
        description: "FIFO queue",
      },
      stack: {
        pattern:
          /(?:^|\s|;|\(|\)|,|&|::)(?:std::)?stack\s*<[^>]+>\s*(\w+)(?:\s*\{[^}]*\})?(?=\s|;|\)|,|$)/g,
        type: "adaptor",
  ...
  const handleStructureClick = (structure) => {
    ...
    const lastStep = [...executionSteps]
      .reverse()
      .find((step) => step.name === structure.variableName && step.state);

    if (lastStep) {
      console.log(
        "DEBUG (CppEditorPage.jsx handleStructureClick): Found lastStep:",
        lastStep
      );
      ...
      if (
        ...
        structureData = Array.isArray(structureData)
          ? structureData
          : [structureData];
      ...
      playIntervalRef.current = setInterval(() => {
        setCurrentStepIndex((prev) => {
          ...
          if (currentStep && currentStep.name) {
            const structure = dataStructures.find(
              (ds) => ds.variableName === currentStep.name
            );
            if (structure) {
              if (currentStep.state) {
                ...
                if (
                  ...
                  const isMinHeap =
                    currentStep.metadata?.comparator === "greater";
                  structureData = {
                    elements: structureData,
                    type: isMinHeap ? "min" : "max",
                  };
              ...
              if (
                !selectedStructure ||
                selectedStructure.variableName !== currentStep.name
              ) {
                setSelectedStructure(structure);
                setShowScene(true);
              }
            ...
            <IOSection>
              ...
              <AnalysisSection>
                <Box sx={{ p: 2, backgroundColor: "#9B6B9E", color: "white" }}>
                  <Typography variant="h6">
                    Structuri de date identificate
                  </Typography>
                </Box>
              ...
              {selectedStructure && (
                <VisualizationSection>
                  ...
                  >
                    <Typography variant="h6">
                      {selectedStructure.name} ({selectedStructure.variableName}
                      ) Visualization
                    </Typography>
                    <IconButton
                  ...
                  <Box sx={{ flex: 1, p: 2, height: "calc(100% - 64px)" }}>
                    {selectedStructure.name === "vector" && (
                      <Vector
                        elements={
                          structureStates[selectedStructure.variableName] || []
                        }
                        showControls={false}
                        height="100%"
                        width="100%"
