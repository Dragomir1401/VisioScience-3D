...
const ElasticCollisionFormulas = () => {
  ...
  useEffect(() => {
    const fetchFormulas = async () => {
      ...
      try {
        ...
        setError("Eroare la încărcarea formulelor pentru coliziune elastică.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-white border border-purple-300 p-4 rounded-lg shadow mt-4 text-purple-800">
      <h3 className="text-lg font-semibold border-b border-purple-300 pb-1 mb-2">
        Formule pentru Coliziune Elastică
      </h3>
      {loading ? (
        ...
        <ul className="list-disc list-inside space-y-1">
          {formulas.length === 0 ? (
            <li className="italic text-gray-500">Nu există formule pentru coliziune elastică.</li>
          ) : (
            formulas.map((f) => (
              <li key={f._id}>
...
export default ElasticCollisionFormulas;
