  ...
  useEffect(() => {
    const fetchFormulas = async () => {
      ...
      try {
        ...
        setError("Eroare la încărcarea formulelor pentru pendul.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="bg-white border border-purple-300 p-4 rounded-lg shadow mt-4 text-purple-800">
      ...
      {loading ? (
        ...
        <ul className="list-disc list-inside space-y-1">
          {formulas.length === 0 ? (
            <li className="italic text-gray-500">Nu există formule pentru pendul.</li>
          ) : (
            formulas.map((f) => (
              <li key={f._id}>
...
export default PendulumFormulas;
