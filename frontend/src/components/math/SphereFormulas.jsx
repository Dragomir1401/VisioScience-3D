...
const SphereFormulas = () => {
  ...
  useEffect(() => {
    const fetchFormulas = async () => {
      setLoading(true);
      try {
        const res = await fetch("http://localhost:8000/feed/shape/sphere");
        if (!res.ok) throw new Error(await res.text());
        const data = await res.json();
        setFormulas(Array.isArray(data) ? data : []);
      } catch (e) {
        setError("Eroare la încărcarea formulelor pentru sferă.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="bg-white border border-purple-300 p-6 rounded-lg shadow-md mt-6 space-y-4 text-purple-800">
      ...
      {loading ? (
        ...
        <ul className="list-disc list-inside space-y-2">
          {formulas.length === 0 ? (
            <li className="italic text-gray-500">Nu există formule pentru sferă.</li>
          ) : (
            formulas.map((f) => (
              <li key={f._id}>
      ...
      <p className="text-sm italic text-purple-700">
        Unde <span className="font-bold">r</span> este raza sferei.
      </p>
    </div>
  );
};
